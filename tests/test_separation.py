...
class TestSeparateVocalSongDispatcher:
    ...
    def test_dispatches_to_local(self, mock_local, tmp_path):
        ...
        mock_local.assert_called_once()
        assert "lead_vocals" in result
        assert result["backing_vocals"] is None  # local fallback
...
class TestModalConstants:
    """Test Modal module constants for MelBand Roformer configuration."""

    def test_karaoke_model_constants_defined(self):
        """Karaoke model checkpoint and config constants should be defined."""
        from musicmixer.services.separation_modal import (
            MELBAND_KARAOKE_CKPT,
            MELBAND_KARAOKE_CONFIG,
            MELBAND_KARAOKE_HF_REPO,
        )

        assert MELBAND_KARAOKE_CKPT.endswith(".ckpt")
        assert MELBAND_KARAOKE_CONFIG.endswith(".yaml")
        assert "/" in MELBAND_KARAOKE_HF_REPO  # HuggingFace repo format

    def test_vocals_model_constants_defined(self):
        """Vocals model checkpoint and config constants should be defined."""
        from musicmixer.services.separation_modal import (
            MELBAND_VOCALS_CKPT,
            MELBAND_VOCALS_CONFIG,
            MELBAND_VOCALS_HF_REPO,
        )

        assert MELBAND_VOCALS_CKPT.endswith(".ckpt")
        assert MELBAND_VOCALS_CONFIG.endswith(".yaml")
        assert "/" in MELBAND_VOCALS_HF_REPO

    def test_msst_weights_dir_is_absolute(self):
        """MSST weights directory should be an absolute path inside the container."""
        from musicmixer.services.separation_modal import MSST_WEIGHTS_DIR

        assert MSST_WEIGHTS_DIR.startswith("/")
